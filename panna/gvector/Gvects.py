...
class GvectBase():
    def __init__(self,
        ...
        unit2A = self.unit2A
        # RADIAL_COMPONENTS
        self.Rc_rad = Rc_rad * unit2A
        self.Rs0_rad = Rs0_rad * unit2A
    ...
    @property
    def unit2A(self):
        if self.param_unit in ['a', 'angstrom', 'A', 'Angstrom', 'ANGSTROM']:
            unit2A = np.float32(1)
        elif self.param_unit in ['au', 'bohr', 'bohr_radius', 'BOHR']:
            unit2A = np.float32(1) * BOHR2A
        else:
            raise UnitError('unit not recognized')
        return unit2A

    @property
    def Rsst_rad(self):
        if self._Rsst_rad != None:
...
class GvectmBP(GvectBase):
    ...
    def __call__(self, key, positions, species, lattice_vectors, **kwargs):
        ...
        Return:
            (key, gvector as list, dgvecotor as a list if required)
        ...
        if self.compute_dGvect:
            return calculate_Gvector_dGvect_mBP(
                key, positions, species, lattice_vectors,
                self.number_of_species, self.eta_rad, self.Rc_rad,
                self.Rs0_rad, self.RsN_rad, self.Rsst_rad, self.eta_ang,
                self.Rc_ang, self.Rs0_ang, self.Rsst_ang, self.RsN_ang,
                self.zeta, self.ThetasN, **kwargs)
        else:
            ...
            for idx in range(n_atoms):
                ...
                for atom_kind_1 in range(n_species):
                    for atom_kind_2 in range(atom_kind_1, n_species):
                        ...
                        species_idxs_1 = np.where(
                            cutoff_species == atom_kind_1)
                        species_idxs_2 = np.where(
                            cutoff_species == atom_kind_2)
                        ...
                        kind1_kind2_angular_g = G_angular_mBP(
                            ...
                            sampling_rad_ang[np.newaxis, np.newaxis, :, np.
                                             newaxis],
                            sampling_theta[np.newaxis, np.newaxis, np.
                                           newaxis, :], eta_ang, zeta, Rc_ang)
                ...
                for atom_kind_1 in range(n_species):
                    for atom_kind_2 in range(atom_kind_1, n_species):
                        ...
                        kind1_kind2_angular_g = G_angular_BP(
                            cutoff_r_ij[:, np.newaxis, np.newaxis, np.
                                        newaxis, np.newaxis],
                            cutoff_r_ik[np.newaxis, :, np.newaxis, np.
                                        newaxis, np.newaxis],
                            cutoff_r_jk[:, :, np.newaxis, np.newaxis, np.
                                        newaxis],
                            theta_ijk[:, :, np.newaxis, np.newaxis, np.
                                      newaxis],
